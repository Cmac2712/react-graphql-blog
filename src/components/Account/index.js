...
const UPDATE_USER_INFO_MUTATION = gql`
  mutation UPDATE_USER_INFO_MUTATION($screenName: String, $avatar: String) {
    updateUserInfo(screenName: $screenName, avatar: $avatar) {
      id
      screenName
      avatar
    }
...
class Account extends Component {
  ...
  uploadImage = async () => {
    ...
    const res = await fetch(
      "https://api.cloudinary.com/v1_1/cmacintyre/image/upload",
      {
        method: "POST",
        body: data,
      }
  ...
  render() {
    return (
      <AccountFormWrapper className="cms-section min-height">
                        ...
                        <fieldset>
                          <div
                            className={`avatar-wrapper ${
                              this.state.uploading && `image-uploading`
                            }`}
                          >
                            <Avatar
                              onClick={() => {
